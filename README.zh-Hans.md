...
TinyLogger 采用基于 TSC（rdtsc）的高精度计时方式，并通过 CPU 绑定（CPU pinning）与基线对照（baseline）来确保测试结果稳定可靠。

### 配置

编译选项：-O3 -march=native
CPU 频率：约 2.50 GHz（≈ 2.50 cycles/ns）
单核绑定运行（taskset / pthread_setaffinity_np）
使用 NullPrinter 排除 I/O 干扰
批量测量（batch=64）减少计时噪声
