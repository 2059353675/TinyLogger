...
- [配置方法](#配置方法)
    - [配置项说明](#配置项说明)
    - [Console Printer 配置](#Console-Printer-配置)
    - [File Printer 配置](#File-Printer-配置)
- [API 参考](#api-参考)
...
| 配置项                             | 类型                   | 默认值       | 说明                              |
|---------------------------------|----------------------|-----------|---------------------------------|
| `set_buffer_size(size)`         | `size_t`             | `256`     | 环形缓冲区大小（必须为 2 的幂次）              |
| `set_overflow_policy(policy)`   | `OverflowPolicy`     | `Discard` | 溢出策略：`Discard`（丢弃）或 `Block`（阻塞） |
| `add_console_printer(level)`    | `LogLevel`           | `Info`    | 添加控制台输出，设定最低日志级别                |
| `add_file_printer(path, level)` | `string`, `LogLevel` | `Debug`   | 添加文件输出，设定路径与最低级别                |
| `add_null_printer()`            | -                    | -         | 添加 Null Printer（丢弃所有日志，常用于测试）   |
...
注意：日志参数必须是可拷贝的小型 POD 类型（如整数、浮点、C 字符串 `const char*` 等），且总大小不超过固定存储（默认
128B），避免传入临时对象或大对象，如 `std::string`。
